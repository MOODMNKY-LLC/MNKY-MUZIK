...
interface MNKYRecommendsProps {
  likedCount?: number;
}

export function MNKYRecommends({ likedCount = 0 }: MNKYRecommendsProps) {
  return (
    <section className="mt-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {likedCount > 0 && (
          <Link
            href="/liked"
            className="group flex flex-col items-center gap-y-3 p-4 rounded-lg bg-neutral-800/50 hover:bg-neutral-800 transition ring-2 ring-emerald-500/40"
          >
            ...
            <span className="text-neutral-400 text-xs">{likedCount} tracks</span>
          </Link>
        )}
        {MOOD_ITEMS.map((item) => (
          <Link
